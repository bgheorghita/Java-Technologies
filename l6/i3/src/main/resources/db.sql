...
create table assigned_projects (
       ...
       foreign key (student_id) references students(id),
       foreign key (project_id) references projects(id),
       unique key unique_project_assignment (project_id)
);
...
insert into projects(name, category_id, description, deadline) values('Fitness and Health Challenge', 3, 'Embark on a fitness and health journey by setting specific health and fitness goals, such as weight loss, muscle gain, or increased endurance. Document your progress and lifestyle changes.', '2028-02-15');
